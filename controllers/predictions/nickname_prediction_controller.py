...
class NicknameAccumulator:
    ...
    async def process_nicknames(self):
        ...
        if (split[0].lower() != prepend.lower() and (len(old_name) + len(prepend) + 1) <= 32):  # Only proceed if the user doesn't have the tag already and the tag would fit
            try:
                ...
                LOG.error(f"[NicknameAccumulator (PID: {self.id})] Couldn't set nickname of user {member.id}. We are forbidden from editing the member.")
            except Exception as e:
                LOG.error(f"[NicknameAccumulator (PID: {self.id})] Couldn't set nickname of user {member.id}. {e}")
    ...
    async def process_reset(self):
        ...
        old_name = member.display_name
        split = old_name.split(" ")
        if (split[0].lower() == prepend.lower()):  # Only proceed if the user has the tag already
            try:
                await member.edit(nick=old_name.replace(f"{prepend}", "", 1))
            except Forbidden:  # This should only ever happen if we try to edit the Guild Owner
                LOG.error(f"[NicknameAccumulator (PID: {self.id})] Couldn't reset nickname of user {member.id}. We are forbidden from editing the member.")
            except Exception as e:
                LOG.error(f"[NicknameAccumulator (PID: {self.id})] Couldn't reset nickname of user {member.id}. {e}")